...
class WideAndDeep(nn.Module):
    def __init__(self, embedding_dim=64, hidden_dim=128):
        ...
        self.wide_fc = nn.Linear(23, embedding_dim)
        # Deep part (neural network for categorical attributes)
        self.air_id_embedding = nn.Embedding(288, hidden_dim)
        self.deep_fc1 = nn.Linear(hidden_dim, embedding_dim)
        self.deep_fc2 = nn.Linear(embedding_dim, embedding_dim)

    def forward(self, attr):
        # Continuous attributes
        continuous_embed = attr[:, 1:24]
        air_id = attr[:, 0].long()
        unique_air_ids, air_id_indices = torch.unique(air_id, return_inverse=True)
        # Wide part
        wide_out = self.wide_fc(continuous_embed)
        airid_embed = self.air_id_embedding(air_id_indices)
        # Deep part
        deep_out = F.relu(self.deep_fc1(airid_embed))
